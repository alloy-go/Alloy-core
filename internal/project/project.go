...
// Not using Anywhere
func (p *Service) GetProjectsByUser(ctx context.Context, userID string) ([]Project, error) {
...
func (ps *Service) GetProjectsWithDeploymentInfo(ctx context.Context, userID string) ([]ProjectWithDeploymentInfo, error) {
	query := `
	...
	var projects []ProjectWithDeploymentInfo
	for rows.Next() {
		var p ProjectWithDeploymentInfo
		err := rows.Scan(
			&p.ProjectId,
			&p.ProjectName,
			&p.ContextName,
